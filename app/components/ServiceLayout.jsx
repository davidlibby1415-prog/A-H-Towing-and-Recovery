...
// Outer green frame that keeps an even border around the TikTok phone
function TikTokFramedCard({ id, title }) {
  return (
    <div className="relative w-full max-w-[360px]">
      <div className="rounded-[32px] border-2 border-emerald-400/90 bg-black/90 p-[6px] shadow-2xl shadow-black/70">
        <div className="rounded-[28px] bg-black flex justify-center overflow-hidden">
          <TikTokEmbed id={id} title={title} />
        </div>
      </div>
    </div>
  );
            ...
            <div className="space-y-6">
              <VibrantBorder>
                <div className="rounded-[18px] border border-yellow-400/80 bg-black/85 p-4 text-white shadow-[0_18px_40px_rgba(0,0,0,0.9)]">
                  ...
                  {/* BIG TikTok frames – one per row, centered, phone-style inside green frames */}
                  <div className="grid grid-cols-1 gap-4">
                    {TIKTOK_VIDEOS.map((vid) => (
                      ...
                      >
                        ...
                        <TikTokFramedCard id={vid.id} title={vid.title} />
                      </div>
                    ))}
                  </div>
